...
impl LendingContract {
    ...
    pub fn record_withdrawal(env: Env, user: Address, amount: i128) {
        ...
        let current = user_deposits.get(user.clone()).unwrap_or(0);
        assert!(current >= amount, "Insufficient balance");

        user_deposits.set(user, current - amount);
        let new_total = total_deposits - amount;

        Self::save_user_deposits(&env, user_deposits);
        Self::save_total_deposits(&env, new_total);
    }
    ...
    pub fn record_borrow(env: Env, user: Address, amount: i128) {
        ...
        let borrowed = user_borrows.get(user.clone()).unwrap_or(0);

        let max_borrow = deposited * 70 / 100;
        assert!(borrowed + amount <= max_borrow, "Exceeds max borrow limit");

        user_borrows.set(user, borrowed + amount);
    ...
    pub fn record_repayment(env: Env, user: Address, amount: i128) {
        ...
        user_borrows.set(user, borrowed - repay_amount);
